...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _horIndicatorColor = Kcolor16rgb(@"#0099ff", 1);
...
- (void)addTitleBtnArray
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2* NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self tabButtonClicked:_btnArray.firstObject];
    });
...
- (void)selectButton:(UIButton *)button
{
    ...
    _selectBtn = button;
//    JULog(@"ffffffff   %f", _selectBtn.titleLabel.width_extension);

    CGFloat titleLabelWidth = _selectBtn.titleLabel.width_extension;
    
    CGRect frame = _horIndicator.frame;
    
    CGFloat selectButtonX = CGRectGetMinX(_selectBtn.frame);
    
    frame.size.width = titleLabelWidth;
    frame.origin.x = selectButtonX + (_selectBtn.frame.size.width-titleLabelWidth)*0.5;
...
//        button.titleLabel.backgroundColor = [UIColor greenColor];
    ...
    if (_selectBtn) {
        ...
        _selectBtn = [_btnArray firstObject];
    }
    
    CGFloat titleLabelWidth = _selectBtn.titleLabel.width_extension;
    
    _horIndicator.frame = CGRectMake(curIndex*(btnWidth+_titleSpacing)+_edgeInset.left+(btnWidth-titleLabelWidth)*0.5, CGRectGetHeight(self.frame) - _horIndicatorHeight,titleLabelWidth , _horIndicatorHeight);
