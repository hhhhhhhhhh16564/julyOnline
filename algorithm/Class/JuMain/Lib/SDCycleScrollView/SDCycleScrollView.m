...
{
    self.lastPage = [self currentIndex];
...
#pragma makr 自己加的
    ...
    if (self.lastPage == 0) {
        ...
        if (currentPage == self.imagePathsGroup.count) {
            ...
            self.scrollViewDirection = SDCycleScrollViewScrollRight;
            
        }else{
            
            self.scrollViewDirection = SDCycleScrollViewScrollLeft;
    ...
    else if (self.lastPage == self.imagePathsGroup.count){
        ...
        if (self.lastPage < currentPage) {// 左边滑动
            self.scrollViewDirection = SDCycleScrollViewScrollLeft;
        }else{  //右边滑动
            self.scrollViewDirection = SDCycleScrollViewScrollRight;
        }
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:scollViewDirection:toCurrentPage:)]) {
       
        [self.delegate cycleScrollView:self scollViewDirection:self.scrollViewDirection toCurrentPage:currentPage];
    }
